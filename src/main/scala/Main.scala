...
object Main{
  ...
  def getFstDiag(b:Board): Line = {
    for (i <- (0 until b.length).toList) yield
      b(i)(i)
  }
  ...
  def update(p: Player)(ln: Int, col: Int, b: Board) : Board = {
    ...
    for (i <- (0 until b.length).toList) yield
      for (j <- (0 until b.length).toList) yield
        if (i == ln && j == col)
          p
        else
          b(i)(j)
  ...
  def next(p: Player)(b: Board): List[Board] = {
    ...
    val boardsList = {
      indices.flatten.filter(x => x != 0).toList.map(x => x match {
  ...
  def play(b: Board, p: Player): Unit = {
    def humanToTwo(): Unit ={
      var acc = b
      for(i <- (0 until b.size * b.size).toList){
        if(i%2 == 0){
          println("Player One Turn!")
          acc = AI(acc, One)
        } else {
          println("Player Two Turn! Insert coordinates: ")
          val oldAcc = acc
          while(acc == oldAcc) {
            try {
              val Array(x, y) = in.readLine().split(" ").map(_.toInt)
              if (isFree(x - 1, y - 1, acc)) {
                acc = update(Two)(x - 1, y - 1, acc)
              } else {
    ...
    def humanToOne(): Unit ={
      var acc = b
      for(i <- (0 to b.size * b.size).toList){
        if(i%2 == 0){
          println("Player One Turn! Insert coordinates: ")
          val oldAcc = acc
          while(acc == oldAcc) {
            try {
              val Array(x, y) = in.readLine().split(" ").map(_.toInt)
              if (isFree(x - 1, y - 1, acc)) {
                acc = update(One)(x - 1, y - 1, acc)
              } else {
                println("Position already occupied.")
              }
            } catch {
